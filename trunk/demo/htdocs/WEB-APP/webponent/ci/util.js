(function(ci) {
	 ...
	 * @param userConfig
	 */
	self.setNavigator = function(userConfig) {
		...
		var box = config['box'].empty();

		var method = config['method'];

		//box.append('<div class="btn_left"></div><div class="paging"><ul><li class="mr4 j-first"/><li class="j-prev"/><li class="num"/><li class="j-next"/><li class="ml4 j-last"/></ul></div><div class="btn_right">');

		var pagingDiv = $('<div class="paging"></div>');
		var totalnum = parseInt(config['totalNum'], 10);
		var currPage = parseInt(config['currPage'], 10);
		var pageCount = parseInt(config['pageCount'], 10);
		var page = parseInt(config['totalPageNum'], 10);
		...
		// First의 처리
		/*if (currPage == 1) {
			firstHTML += '<button type="button" class="first" disabled="disabled"><<</button>';
		} else {
			firstHTML += '<button type="button" class="first" data-page="1"><<</button>';
		}
*/
		// Prev의 처리
		iDecNum = parseInt((currPage - 1) / pageCount, 10);		
		if (iDecNum > 0) {
			var i = (iDecNum - 1) * pageCount + 1;			
			prevHTML += '<a href="#" class="prev" data-page="'+i+'"><img src="/img/common/arrow_prev.gif" alt="이전으로"></a>';
		} else {			
			prevHTML += '<a href="#" class="prev"><img src="/img/common/arrow_prev.gif" alt="이전으로"></a>';
		}

		// page번호 출력처리
		for ( var i = (iDecNum * pageCount + 1); i <= (iDecNum * pageCount + pageCount); i++) {
			// 출력 할 페이지가 없으면
			if (i > page) {
				break;
			}			
			// 자신의 페이지는 선택이 안되도록 함.
			if (i == currPage) {				
				numHTML += '<a href="#"><strong>' + i + '</strong></a>';
			} else {				
				numHTML += '<a href="#" data-page="'+i+'">' + i + '</a>';
			}			
		}

		// 출력할 페이지가 있을경우 [Next] 출력
		if (((iDecNum + 1) * pageCount) < page) {
			var i = (iDecNum + 1) * pageCount + 1;			
			nextHTML += '<a href="#" class="next" data-page="'+i+'"><img src="/img/common/arrow_next.gif" alt="다음으로"></a>';
		} else {
			nextHTML += '<a href="#" class="next"><img src="/img/common/arrow_next.gif" alt="다음으로"></a>';
		}

		// Last의 처리
		/*if (currPage == page) {
			lastHTML += '<button type="button" class="end" disabled="disabled">>></button>';
		} else {
			lastHTML += '<button type="button" class="end" data-page="' + page + '">>></button>';
		}*/
		
		pagingDiv.append(prevHTML);
		pagingDiv.append(numHTML);
		pagingDiv.append(nextHTML);

		box.append(pagingDiv);
		
		box.find('a:not([data-page])').css('cursor','default');
		box.find('a').on('click', function(e) {			
			if($(this).data('page')){
				method($(this).data('page'));	
			}
			e.preventDefault();
		});

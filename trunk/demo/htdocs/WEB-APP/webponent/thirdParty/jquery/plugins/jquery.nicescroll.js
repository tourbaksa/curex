...
(function(jQuery){
  ...
  var NiceScrollClass = function(myopt,me) {
    ...
    this.opt = {
      doc:$body,
      scrollPosition:undefined,//neoxeni
      gridAutoHeight:false,//neoxeni모바일모드에서 height가 auto일때만 true
      win:false,
      zindex:90,
      cursoropacitymin:0,
      cursoropacitymax:1,
    ...
    this.body = $body;
    //neoxeni추가:S
    this.scrollPosition =this.opt.scrollPosition;
    this.gridAutoHeight =this.opt.gridAutoHeight;
    //neoxeni추가:E
    ...
    //## Check Chrome 22 bug on touch devices -- 더이상 이 버그를 체크하지 않음 역으로 버그가 나옴
    /*
...
*/
      if (!self.ispage || (!cap.cantouch && !cap.isieold && !cap.isie9mobile)) {
        ...
        // Ohsangwon
        if (cap.isios&&!self.ispage&&!self.haswrapper && !$('html').hasClass('iPhone7')) self.css($body,{"-webkit-overflow-scrolling":"touch"});  //force hw acceleration
...
//        
        ...
        if (self.ispage) {
          ...
          if (self.scrollPosition) {
        	  var bd = self.scrollPosition;
        	  var bt = self.win.get(0).offsetTop + 1;
        	  var bl = self.win.get(0).offsetLeft + 0;
        	  
	          if (self.zoom) {
	            self.zoom.css({position:"absolute",top:bt+1,right:0,"margin-right":rail.width+4});
	            bd.append(self.zoom);
	          }
	          rail.css({position:"absolute",top:bt+0});
	          (rail.align) ? rail.css({right:0}) : rail.css({left:0});
	          bd.append(rail);
	          if (railh) {
	            railh.css({position:"absolute",left:bl,bottom:0});
...
//                  trace('END:'+ds+'+'+gap);                  
          ...
          if (cap.cantouch||self.opt.touchbehavior||cap.hasmstouch) {
              ...
              if (e.pointerType&&e.pointerType!=2) return false;
	            ...
	            //*neoxeni		
	      		if(cap.cantouch){
	      			try{
	      				var point =  e;
	          			self.memo.startX = Math.abs(point.pageX);
	          			self.memo.startY = Math.abs(point.pageY);
	          			self.memo.isPrevent = null;
	      			}catch(e){}
	      		}//*/
            ...
            self.ontouchend = function(e) {
              ...
              if (self.rail.drag&&(self.rail.drag.pt==2)) {
            	//*neoxeni doMomentum 시 원래 인자 없음
                self.scrollmom.doMomentum(undefined,self.memo.isPrevent);
                self.rail.drag = false;
                if (self.hasmoving) {
                  self.hasmoving = false;
            ...
            self.ontouchmove = function(e,byiframe) {
              ...
              if (self.rail.drag&&(self.rail.drag.pt==2)) {
                ...
                var fy =0, ny = 0, fx = 0, nx = 0;
                
                //*neoxeni
    			if(self.memo.isPrevent == null){
    				var changeX = Math.abs(self.memo.startX -Math.abs(e.pageX)),
    				changeY = Math.abs(self.memo.startY -Math.abs(e.pageY));
    				self.memo.isPrevent = (changeX - changeY > 0) ? true : false;
    			}
    			
    			if(self.memo.isPrevent){//좌우이동
    				ny = self.rail.drag.st;//원래 y값 
                ...
                //neoxeni 세로스크롤이 없을시는 네이티브 스크롤 사용 원래는 무조건 return;
                if(!self.gridAutoHeight){
                	return self.cancelEvent(e);
                }else if(self.memo.isPrevent && self.gridAutoHeight){
                	return self.cancelEvent(e);
                }
          ...
          if (!self.ispage&&!cap.cantouch&&!(/HTML|BODY/.test(self.win[0].nodeName))) {
            //neoxeni 웹접근성 위배 tabindex 속성부여를 제거
        ...
        if (cap.ischrome&&!self.ispage&&!self.haswrapper) { //chrome void scrollbar bug
          //neoxeni 여기서 width를 강제로 박아서 문제생김..
...
// http://webpro.host.adobe.com/people/jblas/momentum/includes/jquery.momentum.0.7.js  
  ...
  var ScrollMomentumClass2D = function(nc) {
    ...
    //isHMove 좌우이동인지 true,false
    this.doMomentum = function(gp,isHMove) {
      ...
      if (sy||sx) {
        ...
        if(isHMove){
        	self.speedy = undefined;
        }else{
        	self.speedx = undefined;
        }
        onscroll();

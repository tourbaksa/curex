...
FixedColumns.prototype = {
	...
	"_fnConstruct": function ( oInit )
	{
		...
		for (var loop = 0; loop < this.s.iLeftColumns; loop ++) {
			that.s.aiWidths.push(parseInt(this.s.dt.aoColumns[loop].sWidth));
		}

		//sangwon
		/*$('tbody>tr:eq(0)>td', this.s.dt.nTable).each( function (i) {
			console.log(this)
			iWidth = $(this).attr('width');

			that.s.aiWidths.push( parseInt(iWidth) );
		...
		$(this.dom.scroller).scroll( function () {
			...
			that.dom.scroller.scrollTop = that.dom.scroller.scrollTop;
	...
	"_fnGridHeight": function ()
	{
		...
		oGrid.left.body.style.height = $(this.dom.scroller).height() - this.s.dt.oScroll.iBarWidth+"px";
		var scrollbarWidth = this.s.dt.oScroll.iBarWidth;
		oGrid.left.wrapper.style.height = iHeight - scrollbarWidth + 2 +"px";
	...
	"_fnDraw": function ( bAll )
	{
		...
		var clonedLeftHeader = this.dom.clone.left.header;
		var clonedLeftHeaderThs = $(clonedLeftHeader).find('th');
		
		clonedLeftHeaderThs.each(function (i, v) {
	...
	"_fnClone": function ( oClone, oGrid, aiColumns, bAll )
	{
		...
		if ( this.s.dt.aiDisplay.length > 0 )
		{
			...
			for ( iIndex=0 ; iIndex<aiColumns.length ; iIndex++ )
			{
				...
				if (!that.s.aiWidths[iColumn] || that.s.aiWidths[iColumn] < 0) {
					oStyle.width = 0 + "px";
				} else {
	...
	"_fnEqualiseHeights": function ( nodeName, original, clone )
	{
		...
		for ( i=0, iLen=anClone.length ; i<iLen ; i++ )
		{
			...
			iHeightOriginal = $(anOriginal[i]).outerHeight(true);
			iHeightClone = $(anClone[i]).outerHeight(true);
